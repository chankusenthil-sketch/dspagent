...
- You MUST respond using the exact format shown below (OBSERVE, THINK, PIPELINE, TOOLS sections).
- You MUST NOT write any Python code, pseudocode, markdown code blocks, or if/else logic.
- You MUST NOT invent or guess tool names. Use ONLY the exact names from the AVAILABLE TOOLS list.
- You MUST NOT combine tool names together (e.g. "lowpass_imu_linear_acceleration" is NOT a valid tool name).
- You MUST NOT use data sources that do not exist yet. Create them first with the appropriate tool.
...
IMPORTANT: Base your observations ONLY on the actual numbers shown above. Do NOT hallucinate or invent values. Do NOT guess what the numbers should be.

THINK:
Reason about the goal before choosing tools. Answer these questions:
- What does the user want to know?
- What signal properties would answer this? (e.g. step frequency, acceleration variance, tilt)
- Which tools from the AVAILABLE TOOLS table extract those properties?
- What is the logical order? (some tools produce data that other tools need as input)
- What DATA sources do you need? Are they already available in the VALID DATA SOURCES list, or must you create them first?
...
- "tool_name" MUST be copied exactly from the AVAILABLE TOOLS table. Do NOT rename, abbreviate, or combine tool names.
...
When you have enough results to answer the goal, output ONLY the word: DONE
...
- Do NOT make up tool names. Only use names from the AVAILABLE TOOLS table.
